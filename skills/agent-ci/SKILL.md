...
# Exclude downstream/triggered runs
agent-ci pipelines --no-triggered
```
...
# Exit with code 1 if any step failed (useful in scripts)
agent-ci steps --pipeline my-pipeline-slug --exit-status
```
...
```bash
# Wait for all pre-merge pipelines to complete (default 30min timeout)
agent-ci wait

# Wait with a custom timeout and polling interval
agent-ci wait --timeout 60 --interval 30

# Stop immediately on first failure
agent-ci wait --fail-fast

# Wait for only Buildkite pipelines
agent-ci wait --provider buildkite

# Play elevator music while waiting
agent-ci wait --sound

# Exclude downstream/triggered runs
agent-ci wait --no-triggered
```

Options:
- `--timeout <minutes>` — Give up after N minutes (default: 30)
- `--interval <seconds>` — Polling interval, minimum 5 (default: 20)
- `--fail-fast` — Exit immediately when first failure detected
- `--sound` — Play elevator music while waiting
- `--no-triggered` — Exclude downstream/triggered runs

Exit codes: 0 = all passed, 1 = failure detected, 2 = timeout reached.

### List or download artifacts

```bash
# List artifacts for a pipeline
agent-ci artifacts --pipeline my-pipeline-slug

# Filter by glob pattern
agent-ci artifacts --pipeline my-pipeline-slug --pattern "*.json"

# Download artifacts to current directory
agent-ci artifacts --pipeline my-pipeline-slug --download

# Download to a specific directory
agent-ci artifacts --pipeline my-pipeline-slug --download --output-dir ./artifacts

# Download only matching artifacts
agent-ci artifacts --pipeline my-pipeline-slug --download --pattern "coverage/**"

# List artifacts for a specific run
agent-ci artifacts --pipeline my-pipeline-slug --run 1234
```
...
**Note:** When running inside an AI agent (Claude Code, Cursor, Copilot, etc.), the CLI automatically defaults to JSON output for machine-readability. You don't need to pass `--output json` explicitly.
...
- **artifacts**: provider, id, name, path, fileSize, mimeType
...
### Wait for CI and inspect results

```bash
# 1. Wait for CI to complete (fail-fast to save time)
agent-ci wait --fail-fast

# 2. Check final status
agent-ci pipelines --filter failed

# 3. If failures, diagnose (see "Diagnose a CI failure" above)
# 4. Optionally download artifacts (test results, coverage)
agent-ci artifacts --pipeline <slug> --download --pattern "*.xml" --output-dir ./ci-results
```
